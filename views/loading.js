class JSLoadingOverlay {
  ...
  generateSpinnerElement() {
    let emptyDivKey = Object.keys(this.numberOfEmptyDivForSpinner).find(
      (element) => element === this.options.spinnerIcon
    );

    let emptyDivElement = this.generateEmptyDivElement(
      this.numberOfEmptyDivForSpinner[emptyDivKey]
    );
