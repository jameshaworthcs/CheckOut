...
function calculateScores(inst, crs, yr, md, grp, codes, grouped, codeDayFlip) {
  ...
  function calculateScore(code, remainingCodes) {
    ...
    remainingCodes.forEach((otherCode) => {
      if (
        otherCode.codeDay !== moment(Date.now()).format('YYYY-MM-DD') ||
        otherCode.codeState === '0'
      ) {
        ...
        if (
          code.username !== 'anon@checkout.ac.uk' &&
          code.username !== 'guest@checkout.ac.uk' &&
          code.username === otherCode.username &&
          otherCode.codeState === '1'
        ) {
          score++;
        } else if (
          code.username !== 'anon@checkout.ac.uk' &&
        ...
        if (code.username.includes('j-h.ai') || code.username.includes('checkout@jemedia.xyz')) {
          score += 10;
        }
  ...
  const filteredCodes = codes.filter(
    (code) =>
      ...
      code.yr === yr &&
      (code.md === md || md === '%') &&
      (code.groupCode === grp || grp === '%') &&
      (code.codeDay === codeDay || codeDayFlip === false) &&
      code.visState === '1' &&
      code.codeState === '1'
  );
  ...
  const codesWithScores = filteredCodes.map((code) => {
    ...
    let verifiedInfo =
      typeof code.verifiedInfo === 'string'
