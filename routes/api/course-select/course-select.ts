...
app.post('/api/course-select/save', async function (req, res) {
    ...
    if (req.loggedIn) {
        ...
        try {
            ...
            res.json({ success: true, deviceID: req.sessionID, msg: 'Saved course information to account and session. (course-select-api)' });
        } catch (error) {
            console.error(error);
        ...
        res.json({ success: true, deviceID: req.sessionID, msg: 'Saved course information to session. (course-select-api)' });
    }
});
