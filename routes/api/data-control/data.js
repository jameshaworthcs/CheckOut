...
app.post('/api/data/tk/delete', function (req, res) {
    if (req.userState != 'anon') {
        sendEmail(req.useremail, "Data request made [Code logs]", "This is a notification to confirm that a request was made to delete the code logs on file for your CheckOut account. No further action is needed - you will receive an email once the deletion is complete. This can take up to 7 days.")
    }
    ...
    res.json({success: true, msg: "Request submitted. If you are signed in, please check your emails for a confirmation. It can take up to 7 days to process a deletion request."})
});
...
app.post('/api/data/requests/delete', function (req, res) {
    if (req.userState != 'anon') {
        sendEmail(req.useremail, "Data request made [Request logs]", "This is a notification to confirm that a request was made to delete the code logs on file for your CheckOut account. No further action is needed - you will receive an email once the deletion is complete. This can take up to 7 days.")
    }
...
app.post('/api/data/auto/delete', function (req, res) {
    if (req.userState != 'anon') {
        sendEmail(req.useremail, "Data request made [AutoCheckin logs]", "This is a notification to confirm that a request was made to delete the AutoCheckin logs on file for your CheckOut account. No further action is needed - you will receive an email once the deletion is complete. This can take up to 7 days.")
        ...
        res.json({success: true, msg: "Request submitted. Please check your emails for a confirmation. It can take up to 7 days to process a deletion request."})
    } else {
